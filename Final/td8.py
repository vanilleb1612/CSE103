...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 16 21:05:09 2020
...
def finish_sorting(arr, i, j,k):
    while i < j and j < k:
        if arr[i] < arr[j]:
            ...
            prev = arr[i]
            for x in range(i, j):
                #move up one spot each element up to arr[j].
                tmp = arr[x+1]
                arr[x+1] = prev
                prev  = tmp
            arr[i + 1] = last
...
def short_paths(G, n, s):
    dists = [inf] * n #at first, all distances are equal to infinity.
    dists[0] = 0 #Distance to the first node is obviously 0.
    for edge in G:
        if edge[0] == s:
...
#I encountered some problems with the above algorithm, so please find below a second attempt

def short_pathsbis(G, n, s):
    dist = [inf] * n #at first, all distances are equal to infinity.
    dist[0] = 0 #Distance to the first node is obviously 0.
    n=len(G)
    for x in range(n):
        for y in range(n):
            u, v, w = G[j]
...
def test_q3bis():
    ...
    assert (short_pathsbis(g1, 4, 0) == [0, 5, 4, 14])
    assert (short_pathsbis(g2, 5, 0) == [0, -1, 2, -2, 1])
    assert (short_pathsbis(g3, 6, 0) == [0, 9, 3, 12, 16, 8])
    print("Algorithm short_paths works")
